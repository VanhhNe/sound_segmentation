...
class EncoderBlockRes1B(nn.Module):
    def __init__(
        ...
        self.pooling_att = Pooling_attention(out_channels)
    ...
    def forward(self, input_tensor: torch.Tensor, film_dict: Dict) -> torch.Tensor:
        ...
        encoder_pool = self.pooling_att(encoder)
...
class DecoderBlockRes1B(nn.Module):
    def __init__(
        ...
        self.att = Attention_block(in_channels, in_channels, in_channels//2)
        self.att2 = CBAM(in_channels)
        self.pw = nn.Conv2d(in_channels*2, in_channels,kernel_size=1)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.dw = AxialMixer(in_channels, mixer_kernel = kernel_size)
        self.act = nn.GELU()
        self.pw2 = nn.Conv2d(in_channels, out_channels, kernel_size=1)
